...
import dnnlib
from training_avatar_texture_st2 import training_loop
...
def init_dataset_kwargs(opts):
    try:
        if opts.dataset_type == 'image':
            dataset_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture_st2.dataset.ImageFolderDataset', path=opts.data, mesh_path=opts.rdata, use_labels=True, max_size=None, xflip=False, load_exp=opts.gen_exp_cond, load_lms=opts.load_lms)
        elif opts.dataset_type == 'video':
            sampling_dict = {
                'fps': opts.fps,
                'type': opts.type,
                'max_dist': opts.max_dist,
                'num_frames_per_video': opts.num_frames_per_video
            }
            dataset_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture_st2.dataset.VideoFramesFolderDataset', path=opts.data, mesh_path=opts.rdata, use_labels=True, max_size=None, xflip=False, load_exp=opts.gen_exp_cond, 
                                                load_n_consecutive=opts.load_n_consecutive, load_n_consecutive_random_offset=opts.load_n_consecutive_random_offset, max_num_frames=opts.max_num_frames, discard_short_videos=opts.discard_short_videos, sampling_dict=sampling_dict)
...
@click.option('--imgsnap',         help='How often to save snapshots', metavar='TICKS',            type=click.IntRange(min=1), default=5, show_default=True)
@click.option('--netsnap',         help='How often to save snapshots', metavar='TICKS',            type=click.IntRange(min=1), default=10, show_default=True)
...
@click.option('--load_lms', help='If true, load 2d landmarks.', metavar='BOOL',  type=bool, required=False, default=False)
...
@click.option('--discriminator_version',    help='Version of discrminator module', metavar='STR',  type=str, required=False, default=None)

### video configs
@click.option('--dataset_type', metavar='STR', type=str, default='image', required=False)
@click.option('--fps', metavar='INT', type=int, default=25, required=False)
@click.option('--type', metavar='STR', type=str, default='random', required=False)
@click.option('--max_dist', metavar='INT', type=int, default=32, required=False)
@click.option('--max_num_frames', metavar='INT', type=int, default=1024, required=False)
@click.option('--num_frames_per_video', metavar='INT', type=int, default=1, required=False)
@click.option('--load_n_consecutive', metavar='BOOL', type=bool, default=False, required=False)
@click.option('--load_n_consecutive_random_offset', metavar='BOOL', type=bool, default=True, required=False)
@click.option('--discard_short_videos', metavar='BOOL', type=bool, default=False, required=False)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=50, z_noise_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict())#TODo
    c.D_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture_st2.networks_stylegan2.Discriminator', block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    c.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
    c.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture_st2.loss.StyleGAN2Loss')
    ...
    c.image_snapshot_ticks = opts.imgsnap
    c.network_snapshot_ticks = opts.netsnap
    ...
    c.G_kwargs.class_name = f'training_avatar_texture_st2.triplane.HexPlaneGenerator' if opts.model_version is None else f'training_avatar_texture_st2.triplane_{opts.model_version}.HexPlaneGenerator'
    c.D_kwargs.class_name = 'training_avatar_texture_st2.dual_discriminator.DualDiscriminator' if opts.discriminator_version is None else f'training_avatar_texture_st2.dual_discriminator.{opts.discriminator_version}'
    ...
    if c.training_set_kwargs.resolution == 512:
        sr_module = 'training_avatar_texture_st2.superresolution.SuperresolutionHybrid8XDC'
    elif c.training_set_kwargs.resolution == 256:
        sr_module = 'training_avatar_texture_st2.superresolution.SuperresolutionHybrid4X'
    elif c.training_set_kwargs.resolution == 128:
        sr_module = 'training_avatar_texture_st2.superresolution.SuperresolutionHybrid2X'
    else:
    ...
    if opts.cfg == 'ffhq':
        rendering_options.update({
            ...
            'ray_start': 20.0, # near point along each ray to start taking samples.
            'ray_end': 25.0,  # far point along each ray to stop taking samples.
            'box_warp': 1, # the side-length of the bounding box spanned by the tri-planes; box_warp=1 means [-0.5, -0.5, -0.5] -> [0.5, 0.5, 0.5].
            'avg_camera_radius':22.5, # used only in the visualizer to specify camera orbit radius.
    ...
    c.loss_kwargs.num_frames = opts.num_frames_per_video
