...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self._mesh_raw_idx = np.arange(mesh_num, dtype=np.int64)
    ...
    def __getitem__(self, idx):
        # mesh_id = (idx % 3744) // 72 + 52 * (idx // 3744) #TODO :driven
        # mesh_id = idx // 14
        # mesh_id = (idx % 14400) // 72#todo :test & train
        # mesh_id = idx
        mesh_id = 0
        # mesh_id = idx % 8
        # mesh_id = idx
        ...
        if self.load_lms:
            lms = self._load_raw_lms(self._mesh_raw_idx[mesh_id])
        ...
        if self._xflip[idx]:
            ...
            image = image[:, :, ::-1]
        if self.load_obj:
            return image.copy(), self.get_label(idx), verts.copy(), self.get_id(idx), self.have_hair(idx), self.have_gt(idx)
        else:
            return image.copy(), self.get_label(idx)

    def get_id(self, idx):
        # id = (idx % 14400) // 144 #todo:test& train
        # id = idx // 3744#TODO: driven
        # id = 4 #rodin 01 fit
        id = idx // 3744
        return id + 95

    def have_hair(self, idx):
        if idx < 1:
            # return 0#TODO driven
            return 1#TODO train&test&fit
        else:
            return 1#todo

    def have_gt(selfself, idx):
        if idx < 3240:
            return 1
        else:
            if (idx % 3240) % 72 == 30:
    ...
    def get_vert(self, idx):
        ...
        if self.mesh_type == '.obj':
            ...
            verts = self._load_raw_verts_ply(self._mesh_raw_idx[mesh_id])
        else:
            raise NotImplementedError
        if self.load_lms:
            lms = self._load_raw_lms(self._mesh_raw_idx[mesh_id])
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._mesh_path):
            ...
            # self._all_mesh_fnames = self._all_mesh_fnames[:2600]
        ...
        PIL.Image.init()
        # /media/hyx/data2/Projects/Next3d_1013_fit/data/1802_fs020/mixed3
        # / home / hyx / 桌面 / need / fit / w_o_hair / meshes
        ...
        self.vertmean = np.zeros((2548,3)) # 5023
        self.vertstd = 1.0
        ...
        super().__init__(name=name, raw_shape=raw_shape, mesh_num=len(self._all_mesh_fnames) , **super_kwargs)#TODO
    ...
    def _load_raw_labels(self):
        ...
        # for train&test
        # fname = 'hair_split_mead.json'
        # fname_exp = 'hair_split_exp.json'

        fname = 'dataset_mead_scale9_focal2.json'#TODO driven
