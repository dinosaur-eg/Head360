...
def init_dataset_kwargs(opts):
    try:
        if opts.dataset_type == 'image':
            dataset_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture.dataset.ImageFolderDataset', path=opts.data, mesh_path=opts.rdata, use_labels=True, max_size=None, xflip=False, load_exp=opts.gen_exp_cond, load_lms=opts.load_lms)
            ...
            dataset_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture.dataset.VideoFramesFolderDataset', path=opts.data, mesh_path=opts.rdata, use_labels=True, max_size=None, xflip=False, load_exp=opts.gen_exp_cond, 
...
@click.option('--kimg',         help='Total training duration', metavar='KIMG',                 type=click.IntRange(min=1), default=2, show_default=True)
@click.option('--tick',         help='How often to print progress', metavar='KIMG',             type=click.IntRange(min=1), default=1, show_default=True)
@click.option('--imgsnap',         help='How often to save snapshots', metavar='TICKS',            type=click.IntRange(min=1), default=1, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs.class_name = f'training_avatar_texture.triplane.HexPlaneGenerator' if opts.model_version is None else f'training_avatar_texture.triplane_{opts.model_version}.HexPlaneGenerator'
    c.D_kwargs.class_name = 'training_avatar_texture.dual_discriminator.DualDiscriminator' if opts.discriminator_version is None else f'training_avatar_texture.dual_discriminator.{opts.discriminator_version}'
